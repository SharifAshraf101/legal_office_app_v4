...
async function handleSplitDecision(request: Request, env: Env): Promise<Response> {
  ...
  const systemPrompt =
    ...
    '• צור משימה אך ורק כאשר ההחלטה מחייבת אותנו — כלומר את לקוח המשרד/עורך הדין הנ"ל — לבצע פעולה דיונית של תגובה: "להגיב על", "להשיב ל-", "לנמק את", להגיש תגובה/תשובה/הבהרה/התייחסות/כתב טענות — בדרך כלל בתוך מועד.\n' +
    '• אל תיצור משימה עבור חיוב כספי (תשלום סכום, הוצאות, מזונות) — חיוב כספי אינו משימה; השאר task_title ריק.\n' +
    '• אל תיצור משימה כאשר החובה מוטלת על הצד שכנגד ולא עלינו, או כאשר ההחלטה היא קביעה/הכרעה בלבד שאינה דורשת מאיתנו פעולת תגובה דיונית; השאר task_title ריק.\n\n' +
